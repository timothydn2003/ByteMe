...
const Courses = (props) => {
    ...
    const addCourse = async() => {
        ...
        await addDoc(courseCollectionRef, {name: courseName, description: courseDesc, date: new Date(8.64e15).toString()})
                    ...
                    <Row>
                        <Col style={{disply:'flex', justifyContent:'left'}}>
                            {courses.map((data) => {
                                return(
                                    <div>
                                        <button onClick={() => setCurrentCourse(data)} className='courseBtn'>{data.name}</button><br/>
                                    </div>
                                )
                            })}
                        </Col>
