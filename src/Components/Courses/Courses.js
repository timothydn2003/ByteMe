...
const Courses = (props) => {
  ...
  const { addCourse } = React.useContext(Context);
  ...
  useEffect(() => {
    const getCourses = async () => {
      ...
      const tmpArr = data.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      }));
  ...
  const addCourseHelper = async (e) => {
    e.preventDefault();
    handleClose();
    props.setLoading(true);
    await addCourse(courseName, courseDesc);
